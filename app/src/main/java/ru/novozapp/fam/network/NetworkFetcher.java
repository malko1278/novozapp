package ru.novozapp.fam.network;
...
import ru.novozapp.fam.db.DatabaseManager;
...
public class NetworkFetcher {
    ...
    public static JSONObject executeRequest(String query, boolean useOffline) {
        ...
        try {
            ...
            if (useOffline)   dbNetwork = DatabaseManager.getInstance().findNetworkQuery(query);
            if (dbNetwork == null) {
                URL url = null;
                if (query.indexOf(";") > -1)    url = new URL(query.split(";")[1]);
                else    url = new URL(query);
            ...
            } else   response = dbNetwork.getValue();
